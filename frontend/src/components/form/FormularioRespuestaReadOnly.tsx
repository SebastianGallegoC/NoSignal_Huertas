...
const rowClass = 'flex flex-col gap-0.5 border-b border-slate-100 py-2.5 last:border-b-0 sm:flex-row sm:items-start sm:justify-between sm:gap-4';
      ...
      <dl className="border-t border-slate-100 px-4 pb-3 pt-1">
        {fieldKeys.map((key) => (
          <div key={key} className={rowClass}>
            ...
            <dd className="min-w-0 text-sm text-slate-900 sm:text-right">{displayFieldValue(key, datos[key])}</dd>
          </div>
        ))}
      </dl>
    </details>
...
export const FormularioRespuestaReadOnly = ({ snapshot }: { snapshot: FormularioSnapshot }) => {
  ...
  if (!hasAnyField && !gps && fotos.length === 0) {
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50/80 px-4 py-3 text-sm text-amber-900">
        No hay respuestas de campos para mostrar (p. ej. registro antiguo sin copia local o datos vacíos en
        servidor).
      </div>
    );
  }
    ...
    <div className="space-y-4 text-slate-800">
      {gps ? (
        <section className="rounded-xl border border-slate-200 bg-slate-50/90 p-4">
          <h3 className="text-sm font-semibold text-slate-900">Ubicación GPS</h3>
          <dl className="mt-2 grid gap-2 text-sm sm:grid-cols-3">
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-slate-500">Latitud</dt>
              <dd className="font-mono text-slate-900">{gps.latitud}</dd>
            </div>
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-slate-500">Longitud</dt>
              <dd className="font-mono text-slate-900">{gps.longitud}</dd>
            </div>
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-slate-500">Precisión (m)</dt>
              <dd className="font-mono text-slate-900">
                {gps.precision != null && gps.precision > 0 ? gps.precision : '—'}
              </dd>
            </div>
          </dl>
          <a
      ...
      {fotos.length > 0 ? (
        <section className="rounded-xl border border-slate-200 bg-white p-4">
          <h3 className="text-sm font-semibold text-slate-900">Fotos ({fotos.length})</h3>
